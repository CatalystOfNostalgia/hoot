...
def insert_comment(item_id, date, relevancy, pleasantness, attention,
    ...
    new_comment = models.comments.Comments(
        item_id=item_id,
        relevancy=relevancy,
...
def insert_media(title, creator, description, media_type, asin, date):
    ...
    new_media = models.multimedia.Multimedia(
        title=title,
        creator=creator,
...
def insert_media_emotion(media_id, emotion):
    ...
    new_emotion = models.multimedia_emotions.MultimediaEmotions(
        media_id=media_id,
        emotion=emotion
    )
...
def remove_media(asin):
    """ Removes the specified media and all associated comments and media. """
...
def find_media_by_asin(asin):
    ...
    media_objs = models.session.query(models.Multimedia).\
...
def find_emotions_for_media(media_id):
    ...
    media_emotions = models.session.query(models.MultimediaEmotions).filter(
        models.MultimediaEmotions.media_id == media_id
    ).all()
...
def find_date_for_review(asin):
    """ Finds the date the review was written. """
    media = find_media_by_asin(asin)
